...
public class Result<T> where T : class
{
    ...
    /// <param name="state">Http status code of the error. Default status code is 400(Bad request)</param>
    /// <returns></returns>
    public static Result<T> Fail(string message, IEnumerable<string> errors,
        HttpStatusCode state = HttpStatusCode.BadRequest)
    {
    ...
    /// <returns></returns>
    public static Result<T> Fail(string message, IEnumerable<string> errors, T data,
        HttpStatusCode state = HttpStatusCode.BadRequest)
    {
    ...
    {
        if (IsError) throw new ProblemsException(Message, Errors);
    }
}